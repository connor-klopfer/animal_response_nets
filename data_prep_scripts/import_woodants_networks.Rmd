...
conditions <- read_csv("data/data Burns_ants food removal/conditions.csv")
...
head(summary)
network_output <- summary %>% tibble::rownames_to_column() %>%
  rename(nodeID=rowname, colony_timepoint=focal.m) %>%
  mutate(species="wood ants") %>%
  select(species, colony_timepoint, nodeID, everything())

write.csv(network_output, file = "network_output_woodants.csv")
